...
function displayQuestions() {
    ...
    if (questions.length === 0) {
        list.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">Nu există întrebări generate încă.</p>';
        return;
    }
    
    list.innerHTML = questions.map(q => `
        <div class="question-item" onclick='viewQuestion(${JSON.stringify(q).replace(/'/g, "&#39;")})'>
